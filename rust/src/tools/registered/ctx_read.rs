        ...
        tool_def(
            ...
            json!({
                ...
                "properties": {
                    ...
                    "mode": {
                        "type": "string",
                        "description": "Recommended (defaults to auto). full=verbatim(edit-ready) anchored=full+N:hh|anchors(edit via ctx_patch) raw=exact-bytes signatures=API map=structure auto=smart diff=cache-delta lines:N-M=window (comma multi-selects: lines:5,10-20; lines:-N=last N lines) reference=quotes task=focus"
                    },
